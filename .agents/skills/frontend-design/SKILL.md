...
## Khi dùng trong Laravel/Blade projects

Khi dùng skill này trong context Laravel/Blade:
- Sau khi tạo được thiết kế HTML/CSS ấn tượng, chuyển tiếp sang skill `stitch-to-blade` hoặc `blade-component` để convert theo convention project.
- Màu sắc phải map sang CSS variables/tokens của project.
- Không dùng inline style trong Blade; đưa style vào file CSS riêng theo `project-config.md`.
- Admin UI phải tuân thủ `doc/admin-design.md` nếu project có file này.
- Nếu thiết kế thể hiện nghiệp vụ cụ thể, đọc `doc/4_business_rule.md` hoặc tài liệu business tương đương trước khi chốt UI.
