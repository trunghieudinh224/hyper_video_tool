...
- Nếu có Laravel/Blade, kết hợp với `blade-component` hoặc `stitch-to-blade` khi cần.
...
4. `.agents/rules/design-presets.md`, `DESIGN.md`, hoặc `doc/admin-design.md` nếu có
5. Blade/HTML/CSS/JS đang cần polish
6. Screenshot/mockup/feedback của người dùng nếu có

Nếu project có CodeGraph, có thể dùng `wf-codegraph-explore` để hiểu flow UI/API trước khi sửa.
...
- Admin/ops: dense, quiet, scan-friendly, restrained color.
- Ecommerce: product-first, trust, price/CTA clarity, rich but not noisy.
- Service/booking: warm, clear packages, easy contact/booking, calm trust.
- Dashboard/analytics: hierarchy, contrast, table/chart readability.
- Landing/client page: visual signal early, brand/object/product rõ trong viewport đầu.
...
- Blade chỉ structure/data binding.
- CSS riêng trong `public/css/...`.
- JS riêng trong `public/js/...`.
- Dùng helper asset của project nếu có, ví dụ `asset_v()`.
