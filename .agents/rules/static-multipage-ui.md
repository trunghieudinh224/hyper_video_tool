...
File này là rule bắt buộc khi tạo, sửa, refactor hoặc review UI trong thư mục `app/`.
...
- Không gom nhiều màn hình vào một `app/index.html` dạng SPA tab ẩn/hiện.
...
- Mỗi màn hình chính phải là một file HTML riêng trong `app/pages/`.
- CSS riêng từng màn hình phải nằm trong `app/styles/pages/`.
- JS riêng từng màn hình phải nằm trong `app/scripts/pages/`.
- Phần dùng chung phải nằm trong `app/styles/` và `app/scripts/common/`.
...
- Hướng ưu tiên: dùng `app/scripts/common/shell.js` render shell chung vào từng page.
- Hướng fallback: dùng `app/shared/base.html` làm template tham chiếu và copy shell nhất quán sang từng page.

`app/shared/base.html` là tài liệu mẫu nội bộ, không phải file browser tự load.
...
- `app/index.html` chỉ là entry gọn: redirect hoặc link tới `app/pages/overview.html`.
...
Không được xóa `app/index.html` cũ hoặc logic cũ trước khi page thay thế đã hoạt động.

## Verification Bắt Buộc

Trước khi bàn giao UI:

- Mở `app/index.html`, xác nhận vào được `pages/overview.html`.
